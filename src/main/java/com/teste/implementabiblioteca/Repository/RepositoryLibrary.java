...
public interface RepositoryLibrary extends JpaRepository<LibraryEntity, Integer> {
    ...
    @Query(value = "Insert into Biblioteca (nome,fk_endereco) values " +
            "(:nome , :fk_endereco) where id_biblioteca= :id_biblioteca",nativeQuery = true)
    Integer save(String nome , Integer fk_endereco,Integer id_biblioteca);

    @Modifying
    @Query(value = "Update Biblioteca set nome= :nome , fk_endereco= :fk_endereco " +
            "where id_biblioteca= :id_biblioteca",nativeQuery = true)
    Integer update(String nome, Integer id_fk_endereco,Integer id_biblioteca);

    @Modifying
    @Query(value = "Delete from Bilbioteca where id_biblioteca= : id_biblioteca",nativeQuery = true)
